...
(define (stm? e)
  (match e
    ...
    [(decr x) (symbol? x)]
    [(for x e1 e2 s1 s2) (and (symbol? x)
                              (expr? e1)
                              (expr? e2)
                              (stm? s1)
                              (stm? s2))]
...
(define (interp p m)
  (match p
    ...
    [(decr x) (env-update x (- (env-lookup x m) 1) m)]))
...
(define (list-to-cycle xs)
  (define (create-mlist ys)
    (if (null? ys)
        ys
        (mcons (car ys) (create-mlist (cdr ys)))))
  (define (get-last-pair xs)
    (if (null? (mcdr xs))
        xs
        (get-last-pair (mcdr xs))))
  (let* ([lista (create-mlist xs)]
        [last (get-last-pair lista)])
    (begin (set-mcdr! last lista)
           lista)))
(define (cycle xs)
  (let ((acc (mcons (car xs) null)))
    (begin (set-mcdr! acc (foldr mcons acc (cdr xs)))
           acc)))
;cwiczenie 2
...
(display (fib-using-WHILE 10))

;;cwiczenie 7
(define (for-macro x e1 e2 s1 s2)
...
(define (n-queens-in-WHILE n)
  (comp
   (apply comp* (map (lambda (i)
                       (assign (gen-symbol "x" i) (const 0))) (range 1 (+ n 1))))
   (var-block 'done (const #f) ...)))
...
(define (queens-using-WHILE n)
  (interp (n-queens-in-WHILE n)
          (env-from-assoc-list (map (lambda (i) (list (gen-symbol "x" i) 42)) (range 1 (+ n 1))))))
